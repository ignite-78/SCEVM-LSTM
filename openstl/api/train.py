...
class BaseExperiment(object):
    ...
    def _preparation(self, dataloaders=None):
        ...
        if self._early_stop <= self._max_epochs // 5:  # 避免设置过小的早停阈值，不想在训练初期启用早停
            self._early_stop = self._max_epochs * 2  # 停用早停机制
    ...
    def display_method_info(self):
        ...
        if self.args.method in ['scevm_lstm', 'predrnn']:
    ...
    def train(self):
        ...
        for epoch in range(self._epoch, self._max_epochs):
            ...
            if epoch % self.args.log_step == 0:
                ...
                if self._rank == 0:
                    ...
                    enable_early_stop = (epoch > self._early_stop)
                    early_stop_flag = recorder(vali_loss, self.method.model, self.path, epoch, early_stop=enable_early_stop)
        ...
        # 设置最好模型的路径例如  work_dirs/cikm_simvp-1e-3/checkpoint.pth
        best_model_path = osp.join(self.path, 'checkpoint.pth')
        # 加载模型权重
        self._load_from_state_dict(torch.load(best_model_path))
        # 个人认为，上面加载了best_model的权重，代表当前服务器当中，加载了最好模型，所以接下来在train.py中调用test函数，进行测试的就是best_model
        # 而不是最后一轮训练后的模型（可能不是最好模型best_model）
    ...
    def test(self):
        ...
        def center_crop(data, target_size=(101, 101)):
            """支持多维数组的中心裁剪（适配N,T,C,H,W格式）"""
            h, w = data.shape[-2], data.shape[-1]
            start_h = (h - target_size[0]) // 2
            start_w = (w - target_size[1]) // 2
            return data[..., start_h:start_h + target_size[0], start_w:start_w + target_size[1]]

        # 加载best_model的参数权重
        if self.args.test:
            best_model_path = osp.join(self.path, 'checkpoint.pth')
            self._load_from_state_dict(torch.load(best_model_path))
        # 触发自定义预处理逻辑（如初始化日志记录、重置评估指标）
        self.call_hook('before_val_epoch')
        # 执行单论测试
        ...
        # 计算评估指标，通过 metric 函数计算评估指标（如 MAE、MSE），并生成日志字符串

        # 在调用metric函数前添加裁剪逻辑
        cropped_preds = center_crop(results['preds'])  # 形状从(4000,10,1,128,128)→(4000,10,1,101,101)
        cropped_trues = center_crop(results['trues'])
        cropped_inputs = center_crop(results['inputs']) if 'inputs' in results else None

        print(results['preds'].shape, results['trues'].shape,results['inputs'].shape)
        print(cropped_preds.shape, cropped_trues.shape, cropped_inputs.shape)
        eval_res, eval_log = metric(cropped_preds, cropped_trues,
        ...
        results['metrics'] = np.array([eval_res['mae'], eval_res['mse'],eval_res['psnr'],eval_res['ssim']])
        ...
        if self._rank == 0:
            ...
            for np_data in ['metrics', 'inputs', 'trues', 'preds']:
                if np_data == 'metrics':
                    ...
                    cropped = center_crop(results[np_data])
                    np.save(osp.join(folder_path, np_data + '.npy'), cropped)
